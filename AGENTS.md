...
- `globalConfig` is **not** deferred when building `global`, so ladder length and indices stay aligned when `steps` changes.
- `systemConfigBase`, `contrastEmphasis`, and `previewTheme` use `useDeferredValue` + `startTransition`; `inputBusy` covers the full deferred window (loading toast).
...
- **Color:** `new Color(...)` from `colorjs.io` + `serializeColor`. Use `SerializedColor.inSrgbGamut` for gamut flags.
