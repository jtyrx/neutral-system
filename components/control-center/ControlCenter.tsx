...
const dockSystemClassName =
  'relative isolate flex w-max max-w-full origin-bottom items-end justify-center gap-6 overflow-visible motion-safe:will-change-transform [--cc-secondary-dock-width:min(26rem,max(9rem,calc(92vw-11.75rem)))]'

const secondaryDockClassName =
  'relative z-0 flex h-60 min-w-0 w-(--cc-secondary-dock-width) origin-left items-stretch rounded-2xl border border-hairline bg-[color-mix(in_oklch,var(--color-surface-overlay)_55%,transparent)] p-8 shadow-[0_18px_48px_-20px_rgb(0_0_0_/_0.45),0_4px_12px_-6px_rgb(0_0_0_/_0.28)] [contain:layout_style_paint] before:absolute before:top-[0.5625rem] before:bottom-[0.5625rem] before:left-[-0.75rem] before:-z-1 before:w-20 before:rounded-full before:bg-[color-mix(in_oklch,var(--color-surface-overlay)_58%,transparent)] before:shadow-[0_10px_26px_-16px_rgb(0_0_0_/_0.5),inset_0_0_0_1px_color-mix(in_oklch,var(--chrome-hairline)_80%,transparent)] before:blur-[0.25px] supports-[backdrop-filter:blur(1px)]:backdrop-blur-[24px] supports-[backdrop-filter:blur(1px)]:backdrop-saturate-125 dark:bg-[color-mix(in_oklch,var(--color-surface-overlay)_48%,transparent)] dark:before:bg-[color-mix(in_oklch,var(--color-surface-overlay)_52%,transparent)] motion-safe:will-change-[transform,opacity,clip-path,filter]'
