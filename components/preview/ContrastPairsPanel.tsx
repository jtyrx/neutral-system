...
function PairTable({label, tokens}: {label: string; tokens: SystemToken[]}) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-[0.65rem] font-medium uppercase tracking-[0.12em] text-white/45">{label}</p>
        ...
        <table className="w-full min-w-[20rem] text-left text-[0.65rem]">
          ...
          <tbody>
            {pairs.map((p) => (
              ...
              >
                <td className="px-2 py-1.5 text-white/85">{p.label}</td>
                <td className="px-2 py-1.5 font-mono tabular-nums text-white/70">
                  {p.ratio.toFixed(2)}∶1
                </td>
                <td className="px-2 py-1.5 text-white/55">{p.bodyLevel}</td>
                <td className="px-2 py-1.5 text-white/55">{p.uiLevel}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function ContrastPairsPanelInner({lightTokens, darkTokens}: Props) {
  return (
    <div className="space-y-6 rounded-2xl border border-white/10 bg-black/25 p-4 sm:p-5">
      <div>
